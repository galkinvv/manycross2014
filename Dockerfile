...
RUN mkdir -p "${RUSTUP_HOME}" && rustup-init -y --profile minimal --no-modify-path --default-toolchain 1.77.2 -t x86_64-pc-windows-gnu -t x86_64-unknown-linux-musl -t aarch64-unknown-linux-musl -t aarch64-unknown-linux-gnu -c rust-src -c rls -c rust-analyzer -c clippy -c rustfmt
